...
@section('content')
    <div class="stock-page-header">
        <div>
            <h4 class="stock-page-title">Edit Stock Adjustment</h4>
            <p class="stock-page-subtitle">Only pending adjustments can be edited.</p>
        </div>
    ...
    <div class="stock-card">
        <form method="POST" action="{{ route('admin.products.stock.update', $stockAjustment) }}" class="stock-form">
            @csrf
            @method('PUT')
            ...
            <div class="stock-form-grid">
                    ...
                    <select name="product_code" id="product_code"
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->code }}"
                                @selected(old('product_code', $stockAjustment->product_code) === $product->code)>
                    ...
                    <select name="warehouse_id" id="warehouse_id"
                        ...
                        @foreach ($warehouses as $warehouse)
                            <option value="{{ $warehouse->id }}"
                                @selected((int) old('warehouse_id', $stockAjustment->warehouse_id) === $warehouse->id)>
                                {{ $warehouse->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="stock-field">
                    ...
                    <input type="date" name="adjustment_date" id="adjustment_date"
                        ...
                        value="{{ old('adjustment_date', \Carbon\Carbon::parse($stockAjustment->adjustment_date)->toDateString()) }}">
                ...
                <div class="stock-field">
                    ...
                    <input type="number" min="0" name="new_quantity" id="new_quantity"
                        ...
                        value="{{ old('new_quantity', $stockAjustment->new_quantity) }}">
                    @error('new_quantity')
                        <span class="stock-error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="stock-field">
                    ...
                    <select name="reason_code" id="reason_code"
                        ...
                        <option value="damage" @selected(old('reason_code', $stockAjustment->reason_code) === 'damage')>Damage</option>
                        <option value="break" @selected(old('reason_code', $stockAjustment->reason_code) === 'break')>Break</option>
                        <option value="other" @selected(old('reason_code', $stockAjustment->reason_code) === 'other')>Other</option>
                    </select>
                    @error('reason_code')
                        <span class="stock-error">{{ $message }}</span>
                    @enderror
                ...
                <div class="stock-field stock-field-full">
                    ...
                    <textarea name="remark" id="remark" rows="3"
                        ...
                        placeholder="Optional notes about this adjustment">{{ old('remark', $stockAjustment->remark) }}</textarea>
                    @error('remark')
                        <span class="stock-error">{{ $message }}</span>
                    @enderror
                </div>
